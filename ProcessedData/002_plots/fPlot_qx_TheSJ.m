function[] = fPlot_qx_TheSJ(Q_dsp55, Q_mec, Q_HyMec, ...
...
axes1 = axes('Parent',figure1,'FontSize',fontS,...
    'FontName','Arial Narrow','GridLineStyle','-',...
    'XTick',5:3:20,...
    'XTickLabel',{'5.0','8.0','11.0','14.0','17.0','20.0'},...
    'YTick',0:0.5:2,...
...
eps_w = 0.0188^2;
eps_D84 = 0.000791^2;
u_Q_dsp = abs(Q_dsp55.*(eps_Q-eps_w/w(3)-0.5*eps_D84/D84));
u_the_dsp = fGetErrTheRel(the_dsp55);
u_Q_HyMec = abs(Q_HyMec.*(eps_Q-eps_w/w(3)-0.5*eps_D84/D84));
u_the_HyMec = fGetErrTheRel(the_HyMec);
u_Q_mec = abs(Q_mec.*(eps_Q-eps_w/w(3)-0.5*eps_D84/D84));
...
if reg
    ...
    Yreg = p1.*Xreg+p2;
    Xx = Xreg./(w(3)*sqrt(D84^3*g));
    plot1(numel(plot1)+1) = plot(Xx,Yreg,...
        'Color',black,...
...
% LINE UP
fMakeXgrid(8:3:17,[0,2],0.5);
fMakeYgrid([5,20],0.5:0.5:1.5,0.5);
% Create xlabel
xlabel('Dimensionless unitary discharge q_* [-]','FontSize',fontS,'FontName','Arial');
% Create ylabel
...
if write2disc
    cd('UnsteadyFigures');
    export_fig qxTheSJ.png -png
    export_fig qxTheSJ.jpg -jpg % use Powerpoint manipulation
    cd ..
    disp('Figure (qxTheSmartJaeggi) written to disc (unsteady figures folder).');
    close all;
end
